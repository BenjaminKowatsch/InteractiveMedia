    ...
    describe('get all groups', () => {
      ...
      it('should get all groups', () => {
        ...
        .then(res => {
          ...
          expect(res.body.success).to.be.true;
          expect(res.body.payload).to.be.an('object');
          expect(res.body.payload.groups).to.be.an('array');
          expect(res.body.payload.groups).to.have.lengthOf(2);
          expect(res.body.payload.groups[0].name).to.equal(groupScenarios[1].createGroup0.name);
          expect(res.body.payload.groups[0].imageUrl).to.equal(groupScenarios[1].createGroup0.imageUrl);
          expect(res.body.payload.groups[0].groupId).to.equal(groupIds[0]);
          expect(res.body.payload.groups[0].createdAt).to.not.be.null;
          expect(res.body.payload.groups[0].countUsers).to.equal(groupScenarios[1].createGroup0.users.length);
          expect(res.body.payload.groups[0].countTransactions).to.equal(0);
          expect(res.body.payload.groups[1].name).to.equal(groupScenarios[1].createGroup1.name);
          expect(res.body.payload.groups[1].imageUrl).to.equal(groupScenarios[1].createGroup1.imageUrl);
          expect(res.body.payload.groups[1].groupId).to.equal(groupIds[1]);
          expect(res.body.payload.groups[1].createdAt).to.not.be.null;
          expect(res.body.payload.groups[1].countUsers).to.equal(groupScenarios[1].createGroup1.users.length);
          expect(res.body.payload.groups[1].countTransactions).to.equal(0);
        });
      });
